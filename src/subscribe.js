...
class SubscribeBlockTracker extends BaseBlockTracker {
  ...
  async _start() {
    if (this._subscriptionId == null) {
      try {
        let blockNumber = await this._call('vns_blockNumber')
        this._subscriptionId = await this._call('vns_subscribe', 'newHeads', {})
  ...
  async _end() {
    if (this._subscriptionId != null) {
      try {
        await this._call('vns_unsubscribe', this._subscriptionId)
  ...
  _handleSubData(err, data) {
    if (data.method === 'vns_subscription' && data.params.subscription === this._subscriptionId) {
